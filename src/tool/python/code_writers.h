...
void write_snake_case(writer &w, std::string_view const &name, F case_func) {
  ...
  for (std::string_view::size_type i = 0; i < name.size(); i++) {
    if (isupper(name[i]) && i > 0) {
      static const std::set<std::string_view> casing_exceptions = {"UInt",
                                                                   "IPAddress"};

      auto sub = name.substr(i - 1);
      if (sub[0] != '_' &&
          std::none_of(casing_exceptions.begin(), casing_exceptions.end(),
...
void write_license(writer &w, std::string_view comment_marker = "//") {
  w.write("% WARNING: Please don't edit this file. It was generated by "
          "Python/WinRT v%\n\n",
          comment_marker, XLANG_VERSION_STRING);
}

void write_python_setup_filenames(writer &w,
...
// All generated try/catch blocks go thru this function in order to have a
// single place to change as we test the binary size of different approaches
...
void write_pywrapper_type(writer &w, TypeDef const &type) {
  w.write("py::wrapper::%::@", bind<write_type_namespace>(type),
          type.TypeName());
}
...
void write_ns_module_exec_init_python_type(writer &w, TypeDef const &type) {
  ...
  w.write("py::winrt_type<%>::python_type = py::register_python_type(module, "
          "_type_name_@, &_type_spec_@, %);\n",
          bind<write_python_wrapper_template_type>(type), type.TypeName(),
          type.TypeName(), bind<write_type_base>(type));
}

void write_ns_module_exec_func(writer &w,
  ...
  w.write("static int module_exec(PyObject* module) noexcept\n{\n");
  {
    ...
    write_try_catch(
        w,
        [&](auto &w) {
          w.write("py::pyobj_handle bases { PyTuple_Pack(1, "
                  "py::winrt_type<py::winrt_base>::python_type) };\n\n");
...
void write_namespace_initialization(writer &w, std::string_view const &ns,
                                    cache::namespace_members const &members) {
  w.write("\n// ----- % Initialization --------------------\n", ns);
...
void write_namespace_module_init_function(writer &w,
...
self->obj%;
    ...
    w.write(format,
            is_ptype(type) ? "self->obj->hash()"
                           : "std::hash<winrt::Windows::Foundation::"
                             "IInspectable>{}(self->obj)",
...
void write_out_param_init(writer &w, method_signature::param_t const &param) {
  call(
      ...
      [&](generic_type_instance const &) { w.write("nullptr"); }, [](auto) {});
}
...
void write_method_param_definition(writer &w, MethodDef const &method,
  ...
  case param_category::pass_array:
    w.write("auto _% = py::convert_to<winrt::com_array<%>>(%);\n",
            ...
            bind<write_convert_to_params>(method, sequence));
    w.write("auto % = winrt::array_view<const %>(_%.data(), _%.data() + "
            "_%.size());\n",
            bind<write_param_name>(param), param.second->Type(),
            bind<write_param_name>(param), bind<write_param_name>(param),
            bind<write_param_name>(param));
    break;
...
void write_method_invoke_context(writer &w, TypeDef const &type,
...
void write_py_tuple_pack(writer &w, std::vector<std::string> const &params) {
  w.write("PyTuple_Pack(%, %)", static_cast<int>(params.size()),
          bind_list<write_detach_param>(", ", params));
}

void write_method_body_contents(writer &w, TypeDef const &type,
                                MethodDef const &method,
                                bool put_property_method = false) {
  XLANG_ASSERT(!is_constructor(method));

  auto write_void_return = [](writer &w, MethodDef const &method,
                              bool set_property_method) {
...
void write_class_new_function(writer &w, TypeDef const &type) {
  w.write("\nstatic PyObject* _new_%(PyTypeObject* type, PyObject* args, "
...
void write_method_overloads(writer &w, TypeDef const &type,
  ...
  enumerate_methods(w, type, [&](auto const &method) {
    if (method.Name() == method_name) {
      ...
      w.write("if (arg_count == %)\n{\n", count_in_param(signature.params()));
      {
        writer::indent_guard g{w};
        write_try_catch(
            w, [&](writer &w) { write_method_body_contents(w, type, method); });
      }
      w.write("}\n");
    }
  });
...
void write_get_property_function(writer &w, TypeDef const &type,
                                 MethodDef const &method) {
...
void write_put_property_function(writer &w, TypeDef const &type,
                                 MethodDef const &method) {
  if (!method)
    return;
  ...
  w.write("\nstatic % @_%(%, PyObject* arg, void* /*unused*/) noexcept\n{\n",
          return_type, type.TypeName(), method.Name(),
          bind<write_method_self_param>(type, is_static(method)));
  {
    writer::indent_guard g{w};
    if (is_ptype(type)) {
...
void write_event_function(writer &w, TypeDef const &type,
                          MethodDef const &method) {
...
else
{
    return nullptr;
        ...
        w.write(format, bind<write_method_invoke_context>(type, MethodDef{}),
                bind<write_method_invoke_context>(type, MethodDef{}),
                bind<write_method_invoke_context>(type, MethodDef{}));
      },
      "nullptr");
...
void write_method_functions(writer &w, TypeDef const &type) {
  ...
  enumerate_methods(w, type, [&](auto const &method) {
    XLANG_ASSERT(contains(method_map, method.Name())
  ...
  if (!(is_ptype(type) || is_static_class(type))) {
            ...
            type.TypeName());
    {
      writer::indent_guard g{w};
      write_try_catch(w, [&](writer &w) {
        auto format = "auto return_value = "
                      "py::convert_to<winrt::Windows::Foundation::IInspectable>"
  ...
  if (implements_iclosable(type)) {
    w.write(strings::enter_function, type.TypeName(),
            bind<write_pywrapper_type>(type));
  ...
  if (implements_iasync(type)) {
    w.write("\nstatic PyObject* _await_@(%* self) noexcept\n{\n",
            type.TypeName(), bind<write_pywrapper_type>(type));
    {
      write_ptype_body("dunder_await()", [&](auto &w) {
  ...
  if (implements_iiterable(type) || implements_iiterator(type)) {
    w.write("\nstatic PyObject* _iterator_@(%* self) noexcept\n{\n",
            type.TypeName(), bind<write_pywrapper_type>(type));
    {
      write_ptype_body("dunder_iter()",
                       [&](auto &w) { write_dunder_iter_body(w, type); });
    }
    w.write("}\n");
  }
  ...
  if (implements_iiterator(type)) {
    w.write("\nstatic PyObject* _iterator_next_@(%* self) noexcept\n{\n",
            type.TypeName(), bind<write_pywrapper_type>(type));
    {
      write_ptype_body("dunder_iternext()",
                       [&](auto &w) { write_dunder_iter_next_body(w, type); });
    }
    w.write("}\n");
  }
  ...
  if (implements_sequence(type)) {
    w.write("\nstatic Py_ssize_t _seq_length_@(%* self) noexcept\n{\n",
            type.TypeName(), bind<write_pywrapper_type>(type));
    {
      write_ptype_body("seq_length()",
                       [&](auto &w) { write_seq_length_body(w, type); });
    }
    w.write("}\n");
    ...
    w.write(
        ...
        type.TypeName(), bind<write_pywrapper_type>(type));
    {
      write_ptype_body("seq_item(i)",
                       [&](auto &w) { write_seq_item_body(w, type); });
    }
    w.write("}\n");

    if (implements_ivector(type)) {
      w.write("\nstatic int _seq_assign_@(%* self, Py_ssize_t i, PyObject* "
              "value) noexcept\n{\n",
              type.TypeName(), bind<write_pywrapper_type>(type));
      {
        write_ptype_body("seq_assign(i, value)",
                         [&](auto &w) { write_seq_assign_body(w, type); });
      }
      w.write("}\n");
    }
  }
  ...
  if (implements_mapping(type)) {
    w.write("\nstatic Py_ssize_t _map_length_@(%* self) noexcept\n{\n",
            type.TypeName(), bind<write_pywrapper_type>(type));
    {
      write_ptype_body("map_length()",
                       [&](auto &w) { write_map_length_body(w, type); });
    }
    w.write("}\n");
    ...
    w.write("\nstatic PyObject* _map_subscript_@(%* self, PyObject* key) "
            ...
            type.TypeName(), bind<write_pywrapper_type>(type));
    {
      write_ptype_body("map_subscript(key)",
                       [&](auto &w) { write_map_subscript_body(w, type); });
    }
    w.write("}\n");

    if (implements_imap(type)) {
      w.write("\nstatic int _map_assign_@(%* self, PyObject* key, PyObject* "
              "value) noexcept\n{\n",
              type.TypeName(), bind<write_pywrapper_type>(type));
      {
        write_ptype_body("map_assign(key, value)",
                         [&](auto &w) { write_map_assign_body(w, type); });
      }
      w.write("}\n");
    }
  }
...
void write_method_table(writer &w, TypeDef const &type) {
  ...
  auto write_row = [&](MethodDef const &method) {
    ...
    w.write("{ \"%\", (PyCFunction)@_%, %%, nullptr },\n",
            bind<write_lower_snake_case>(method.Name()), type.TypeName(),
            method.Name(), argument_convention_flag, static_flag);
  };
  ...
  w.write("\nstatic PyMethodDef _methods_@[] = {\n", type.TypeName());
  {
    ...
    enumerate_properties(w, type, [&](auto const &prop) {
      auto [get_method, put_method] = get_property_methods(prop);
      // instance properties are listed in the getset table, not the method
      // table
    ...
    if (!(is_ptype(type) || is_static_class(type))) {
      w.write("{ \"_from\", (PyCFunction)_from_@, METH_O | METH_STATIC, "
              "nullptr },\n",
              type.TypeName());
    }
...
void write_getset_table(writer &w, TypeDef const &type) {
  auto write_row = [&](std::string_view field_name,
    ...
    auto setter =
        setter_name.empty()
            ? "nullptr"
            : w.write_temp("(setter)@_%", type.TypeName(), setter_name);

    // TODO: remove const_cast once pywinrt is updated to target Python 3.7.
    //       pywinrt currently targeting 3.6 because that's the version that
    //       ships with VS 2017 v15.8
    ...
    w.write("{ const_cast<char*>(\"%\"), (getter)@_%, %, nullptr, nullptr },\n",
            bind<write_lower_snake_case>(field_name), type.TypeName(),
            getter_name, setter);
  };
  ...
  w.write("\nstatic PyGetSetDef _getset_@[] = {\n", type.TypeName());
  {
    ...
    if (category == category::struct_type) {
      for (auto &&field : type.FieldList()) {
        write_row(field.Name(), w.write_temp("get_%", field.Name()),
      ...
      enumerate_properties(w, type, [&](Property prop) {
        ...
        // static properties are listed in the method table, not the getset
        // table
        if (!is_static(getter)) {
...
static PyType_Spec _type_spec_@ =
{
  ...
  w.write(format, type_name, bind<write_ns_module_name>(type.TypeNamespace()),
          type_name, type_size, type_name);
}
...
struct pinterface_python_type<%<%>>
{
  ...
  w.write(format, bind_list<write_template_arg>(", ", type.GenericParam()),
          type, bind_list<write_template_arg_name>(", ", type.GenericParam()),
          bind<write_pyproj_type>(type), bind<write_pyimpl_type>(type),
...
void write_pinterface_decl(writer &w, TypeDef const &type) {
  ...
  w.write("\nstruct @\n{\n", type.TypeName());
  {
    ...
    w.write("virtual ~@() {};\n", type.TypeName());
    w.write("virtual winrt::Windows::Foundation::IUnknown const& get_unknown() "
            "noexcept = 0;\n");
    ...
    if (implements_sequence(type)) {
      ...
      if (implements_ivector(type)) {
        w.write("virtual int seq_assign(Py_ssize_t i, PyObject* value) "
                "noexcept = 0;\n");
      }
    }
    ...
    if (implements_mapping(type)) {
      ...
      if (implements_imap(type)) {
        w.write("virtual int map_assign(PyObject* key, PyObject* value) "
                "noexcept = 0;\n");
      }
    }
  }
...
void write_pinterface_impl(writer &w, TypeDef const &type) {
  ...
  w.write("\ntemplate<%>\nstruct @ : public py::proj::%::@\n{\n",
          ...
          type.TypeName(), bind<write_type_namespace>(type), type.TypeName());

  {
    ...
    w.write("@(%<%> o) : _obj(o) {}\n", type.TypeName(), type,
            bind_list<write_template_arg_name>(", ", type.GenericParam()));
    w.write("winrt::Windows::Foundation::IUnknown const& get_unknown() "
            "noexcept override { return _obj; }\n");
    w.write("std::size_t hash() noexcept override { return "
            "py::get_instance_hash(_obj); }\n");
    ...
    enumerate_properties(w, type, [&](auto const &prop) {
      ...
      if (put_method) {
                ...
                put_method.Name());
        {
          writer::indent_guard gg{w};
          write_setter_try_catch(w, [&](writer &w) {
            write_method_body_contents(w, type, put_method, true);
          });
        }
        w.write("}\n");
      }
    ...
    if (implements_iasync(type)) {
      w.write("PyObject* dunder_await() noexcept override { return "
              "py::dunder_await(_obj); }\n");
    }
    ...
    if (implements_sequence(type)) {
      ...
      if (implements_ivector(type)) {
        w.write("int seq_assign(Py_ssize_t i, PyObject* value) noexcept "
                "override\n{\n");
        {
          writer::indent_guard gg{w};
    ...
    if (implements_mapping(type)) {
      ...
      if (implements_imap(type)) {
        w.write("int map_assign(PyObject* key, PyObject* value) noexcept "
                "override\n{\n");
        {
          writer::indent_guard gg{w};
  ...
  call(
      ...
      [&](fundamental_type type) {
        ...
        case fundamental_type::Boolean:
          w.write("p");
          break;
          // TODO: 'u' format string was deprecated in Python 3.3. Need to move
          // to a supported construct
        case fundamental_type::Char:
          w.write("C");
          break;
        case fundamental_type::Int8:
  ...
  call(
      ...
      [&](fundamental_type type) {
        if (type == fundamental_type::String) {
          w.write(", py::convert_to_hstring, &_%", field.Name());
        } else {
  ...
  call(
      ...
      [&](fundamental_type type) {
        if (type == fundamental_type::Char) {
          w.write("static_cast<char16_t>(_%)", field.Name());
        } else if (type == fundamental_type::Int8) {
          w.write("static_cast<int8_t>(_%)", field.Name());
        } else {
          w.write("_%", field.Name());
        }
      },
      [&](type_definition const &type) {
        auto category = get_category(type);
        XLANG_ASSERT((category == category::struct_type) ||
...
void write_struct_field_ref_capture(writer &w, Field const &field) {
  call(
      get_struct_field_semantics(field, true), [&](fundamental_type) {},
      [&]([[maybe_unused]] TypeDef const &type) {
        XLANG_ASSERT(get_category(type) == category::struct_type);
        w.write(", &_%", field.Name());
      },
...
void write_struct_constructor(writer &w, TypeDef const &type) {
  w.write("\nPyObject* _new_@(PyTypeObject* type, PyObject* args, PyObject* "
          "kwds)\n{",
          type.TypeName());
  {
    writer::indent_guard g{w};
...
if (PyDict_Check(arg))
{
)");
      {
        ...
        write_try_catch(w, [&](writer &w) {
          w.write("auto return_value = py::convert_to<%>(arg);\nreturn "
                  "py::convert(return_value);\n",
                  type);
        });
      }
      w.write("}\n");
...
if (!PyArg_ParseTupleAndKeywords(args, kwds, "%", const_cast<char**>(kwlist)%))
{
    return nullptr;
      ...
      w.write(format, bind_each<write_struct_field_keyword>(type.FieldList()),
    ...
    if (has_custom_conversion(type)) {
      ...
      write_try_catch(w, [&](writer &w) {
        w.write(format, type,
                bind<write_struct_field_initalizer>(type.FieldList().first));
      });
    } else {
...
void write_struct_field_name(writer &w, Field const &field) {
  static const std::set<std::string_view> custom_numerics = {
      "Matrix3x2", "Matrix4x4", "Plane",  "Quaternion",
...
void write_struct_getset_function(writer &w, TypeDef const &type,
                                  Field const &field) {
  w.write("\nstatic PyObject* @_get_%(%* self, void* /*unused*/) noexcept\n{\n",
          type.TypeName(), field.Name(), bind<write_pywrapper_type>(type));
  {
    writer::indent_guard g{w};
          ...
          type.TypeName(), field.Name(), bind<write_pywrapper_type>(type));
  {
    ...
    if (has_custom_conversion(type)) {
      write_setter_try_catch(w, [&](writer &w) {
        w.write("custom_set(self->obj, "
                "py::converter<%>::convert_to(arg));\nreturn 0;\n",
...
if (!PyDict_Check(obj))
{
    ...
    for (auto &&field : type.FieldList()) {
      ...
      if (has_custom_conversion(type)) {
        w.write(
            "custom_set(return_value, converter<%>::convert_to(py_%.get()));\n",
...
return [delegate = std::move(_delegate)](%)
{
      ...
      }
      {
        ...
        w.write("winrt::handle_type<py::gil_state_traits> gil_state{ "
                "PyGILState_Ensure() };\n\n");
